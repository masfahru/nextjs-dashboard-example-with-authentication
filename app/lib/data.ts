...
export async function fetchCardData() {
  ...
  try {
    ...
    const invoiceCountPromise = sql`SELECT COUNT(*) FROM invoices`.then(
      (rows) => rows[0],
    );
    const customerCountPromise = sql`SELECT COUNT(*) FROM customers`.then(
      (rows) => rows[0],
    );
    const invoiceStatusPromise = sql`SELECT
         SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END) AS "paid",
         SUM(CASE WHEN status = 'pending' THEN amount ELSE 0 END) AS "pending"
         FROM invoices`.then((rows) => rows[0]);
...
export async function getUser(email: string) {
  ...
  try {
    const user = await sql<
      User[]
    >`SELECT * from USERS where email=${email}`.then((rows) => rows[0]);
    return user;
  } catch (error) {
